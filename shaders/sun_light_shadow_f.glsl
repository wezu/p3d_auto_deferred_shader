...
void main()
    ...
    float shadow= soft_shadow(spot.shadowMap, shadow_uv.xy, shadow_uv.z, bias, 0.001);
    #endif
    if (distance(shadow_uv.xy, vec2(0.5))>0.5)
        shadow=1.0;


    vec4 final=vec4((color*albedo*shadow)+spot.color.rgb*spec, spec+gloss);
