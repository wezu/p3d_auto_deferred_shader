...
class CameraControler (DirectObject):
    def __init__(self, pos=(0,0,3.0), offset=(0, 12, 12), speed=1.0,
                 zoom_speed=1.0, limits=(1.0, 20.0, -50, 40.0)):
        ...
        self.key_map = {'rotate': False, 'forward':False, 'back':False, 'left':False, 'right':False}
    ...
    def update(self, task):
        ...
        if base.mouseWatcherNode.has_mouse():
            ...
            if not self.key_map['rotate']:
                delta=self.last_delta*(0.999-dt)
                if abs(delta[0])<0.003:
                    delta[0]=0
                if abs(delta[1])<0.003:
                    delta[1]=0
